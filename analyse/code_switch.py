...
def code_switch_experiment():
    ...
    for layer in layers_to_process:
        ...
        for lan in target_languages:
            ...
            if layer_key not in top_features or lan not in top_features[layer_key]:
                print(f"Warning: Feature ID for {lan} at {layer_key} not found. Skipping...")
            ...
            mean_full = sum(layer_full_acts) / len(layer_full_acts) if layer_full_acts else 0.0
            mean_iso = sum(layer_isolated_acts) / len(layer_isolated_acts) if layer_isolated_acts else 0.0
    ...
    for lan in target_languages:
        ...
        # Explicitly label the prefix and noun languages in the legend
        plt.plot(
            layers_to_process, 
        ...
        plt.title(f"{noun_name} Feature Activation (Code-Switching from {prefix_name})", fontweight="bold")
        plt.xlabel("Layer Index", fontweight="bold")
        plt.ylabel("Mean Activation", fontweight="bold")
        plt.grid(True, linestyle="--", alpha=0.6)
        plt.legend()

        plt.savefig(os.path.join(save_dir, "activation_plot.png"), bbox_inches="tight", dpi=300)
        plt.savefig(os.path.join(save_dir, "activation_plot.pdf"), bbox_inches="tight")
        plt.close()

    print(f"Experiment complete! All results saved in results/code_switch/{model_name}/")
